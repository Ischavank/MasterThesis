...
def crop_top_half(image_path):
    ...
    # Define the box for the top half (left, upper, right, lower)
    top_half_box = (0, 0, width, height // 2)
    
    # Crop and return the top half
    top_half_img = img.crop(top_half_box)
    return top_half_img

if testing == True:
    # Crop the image to the top half
    cropped_image = crop_top_half(image_path)
    
    # Save cropped image to a temporary file (if the API requires a file path)
    cropped_image_path = "temp_top_half.jpg"
    cropped_image.save(cropped_image_path)
    
    # Pass the cropped image to the API
    data = assess_plant_health(cropped_image_path, api_key, latitude=None, longitude=None)
    print("data", data)
