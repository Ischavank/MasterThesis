            ...
            if decoded and now - last_qr_time > QR_COOLDOWN:
                for obj in decoded:
                    ...
                    if data not in seen_qrs:
                        seen_qrs.add(data)
                        last_qr_time = now
                        label = datetime.datetime.now().replace(microsecond=0).isoformat() + '_' + data

                        file_path = f"photos5/{label}.jpg"
                        cam1.capture_file(file_path)
                        print(f"📷 QR Detected  {data}, with label {label}")
                        if callback_on_qr:
                            threading.Thread(target=callback_on_qr, args=(label,)).start()
